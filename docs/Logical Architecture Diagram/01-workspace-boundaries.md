...
| -------------------- | ----------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| `apps/web`           | React runtime、router、宣传页和公共页面组合、浏览器入口。                                       | Admin pages、UI primitive、app-local CSS system、custom hooks、locale resources、schema definitions、env policy。    |
...
| `packages/i18n`      | locale tree、translation core、React provider、Node request helper。                            | 业务流程、layout、API handler。                                                                                      |
...
- `apps/web`、`apps/web-admin`、`apps/backend`、`apps/backend-admin` 不要导入不属于自己的 i18n scope。
...
flowchart TD
  ...
  B -- public or marketing page --> Web[apps/web]
