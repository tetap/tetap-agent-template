...
Created: 2026-05-11
Closed: 2026-05-11
Task: Move IAM beyond read-only demo surfaces by implementing production-grade user, role, permission, policy, field permission, menu, session, and operation-log management APIs plus admin UI integration.
...
- [x] Phase 2: Extend `@tetap/iam` with mutation-safe management operations, uniqueness checks, protected super-admin guards, policy default-deny behavior, and operation-log helpers.
...
- IAM mutation guards for uniqueness, protected resources, default-deny policy behavior, session invalidation, and operation records.
