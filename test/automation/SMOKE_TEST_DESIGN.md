...
| `src/smoke/backend-admin-iam.smoke.test.ts`    | Boots the admin Fastify app and exercises IAM auth, CRUD, protected delete, sessions.      | Login succeeds, current-user and overview contracts validate, IAM mutations work, protected role deletion is rejected, forced offline returns `40101`.          |
